...
(window as any).hideCookieBanner = () => {
    cookieBanner.classList.remove('show');
};

// Language selector functionality
function initLanguageSelector() {
    const langBtns = document.querySelectorAll('.lang-btn') as NodeListOf<HTMLButtonElement>;
...
function initCardHoverEffects() {
    const cards = document.querySelectorAll('.division-card, .project-card') as NodeListOf<HTMLElement>;
...
function initFloatingAnimation() {
    ...
    heroStats.forEach((stat, index) => {
        (stat as HTMLElement).style.animation = `float 3s ease-in-out ${index * 0.5}s infinite`;
    });
